...
Если бот не запущен, локальная таблица свободности устаревает. Для ручной проверки использовать `scripts/yclients_sync_status.py`; для ручного обновления - `scripts/sync_yclients_records.py --once`.
