...
- A final public endpoint repeat showed one transient GET timeout followed by a successful HTTP 200; `HEAD` still returns the known `501 Unsupported method ('HEAD')`. `live_health_report.py` is `status=ok`, YCLIENTS is fresh, but DB pool initialization still logs repeated Beget PostgreSQL timeouts before falling back to direct connections.
...
# 2026-06-10 - new server deploy cleanup for max.ermantgz.ru

- Added a dedicated new-server deployment runbook for `https://max.ermantgz.ru`: `best2obs/operations/new-server-deploy-max-ermantgz.md`. It covers DNS, Ubuntu packages, `/opt/admin_niz2`, `/etc/best2/best2.env`, Beget CA, systemd, nginx, Certbot, endpoint checks, MAX subscription migration and YooKassa webhook URL migration.
